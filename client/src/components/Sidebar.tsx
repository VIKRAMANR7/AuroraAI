...
export default function Sidebar({ isMenuOpen, setIsMenuOpen }: SidebarProps) {
  ...
  const filteredChats = chats.filter((chat: ChatType) => {
    ...
    return firstMessage ? firstMessage.includes(s) : chat.name.toLowerCase().includes(s);
  });
    ...
    >
      ...
      >
        ...
        <div className="flex flex-col text-sm">
          ...
          <p className="text-xs text-gray-400">Purchase credits to use AuroraAI</p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md">
        ...
        <label className="relative inline-flex cursor-pointer">
          ...
          <span className="absolute left-1 top-1 size-3 bg-white rounded-full transition-transform peer-checked:translate-x-4"></span>
        </label>
      </div>
